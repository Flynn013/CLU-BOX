 ...
 *
 * Ported from MaxFlynn13/goose-android (engine/mcp/McpExtensionManager.kt).
 */
class McpExtensionManager {
    ...
    suspend fun removeExtension(name: String) {
        mutex.withLock {
            ...
            for (tool in tools) {
                if (toolRouting[tool.name] == name) toolRouting.remove(tool.name)
            }
            client?.disconnect()
    ...
    /**
     * Invoke a tool by name, routing to the correct [McpClient].
     */
